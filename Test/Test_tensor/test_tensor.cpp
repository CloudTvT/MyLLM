...
TEST(test_tensor, to_cpu) {
  ...
  t1_cu.to_cpu();
  ASSERT_EQ(t1_cu.device_type(), kuiper_base::DeviceType::kDeviceCPU);
...
TEST(test_tensor, clone_cuda) {
  ...
  ASSERT_EQ(t2_cu.data_type(), kuiper_base::DataType::kDataTypeFp32);
...
TEST(test_tensor, init1) {
  using namespace kuiper_base;
  auto alloc_cu = kuiper_base::CPUDeviceAllocatorFactory::get_instance();

  int32_t size = 32 * 151;

  tensor::Tensor t1(kuiper_base::DataType::kDataTypeFp32, size, true, alloc_cu);
...
TEST(test_tensor, init3) {
  ...
  ptr[0] = 31;
  tensor::Tensor t1(kuiper_base::DataType::kDataTypeFp32, 32, false, nullptr, ptr);
  ASSERT_EQ(t1.is_empty(), false);
...
TEST(test_tensor, init2) {
  ...
  tensor::Tensor t1(kuiper_base::DataType::kDataTypeFp32, size, false, alloc_cu);
