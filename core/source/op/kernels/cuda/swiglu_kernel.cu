...
void swiglu_kernel_cu(const tensor::Tensor& input1, const tensor::Tensor& input2,
  ...
  CHECK(input1.device_type() == kuiper_base::DeviceType::kDeviceCUDA);

  CHECK_EQ(input2.is_empty(), false);
  CHECK(input2.device_type() == kuiper_base::DeviceType::kDeviceCUDA);

  CHECK_EQ(output.is_empty(), false);
  CHECK(output.device_type() == kuiper_base::DeviceType::kDeviceCUDA);
