...
void swiglu_kernel_cpu(const tensor::Tensor& input1, const tensor::Tensor& input2,
  ...
  CHECK(input1.device_type() == kuiper_base::DeviceType::kDeviceCPU);
  CHECK(input2.device_type() == kuiper_base::DeviceType::kDeviceCPU);
  CHECK(output.device_type() == kuiper_base::DeviceType::kDeviceCPU);
