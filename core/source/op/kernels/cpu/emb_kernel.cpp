...
void emb_kernel_normal(const tensor::Tensor& input, const tensor::Tensor& weight,
  ...
  CHECK(input.device_type() == kuiper_base::DeviceType::kDeviceCPU);

  const auto allocator = kuiper_base::CPUDeviceAllocatorFactory::get_instance();
  for (int32_t i = 0; i < input_num; ++i) {
    ...
    if (token > vocab_size) {
      ...
      if (weight.device_type() == kuiper_base::DeviceType::kDeviceCPU) {
        allocator->memcpy(src_ptr, dest_ptr, weight_dim * sizeof(float),
                          kuiper_base::MemcpyKind::kMemcpyCPU2CPU);
      } else {
