...
void mha_kernel(int32_t pos, int32_t head_num, int32_t layer_index, int32_t seq_len, int32_t kv_dim,
                ...
                kuiper_base::DeviceType device_type, CudaConfig* config) {
  int32_t layer_offset = layer_index * seq_len * kv_dim;
  float scale = 1.f / std::sqrt(static_cast<float>(head_size));

  std::shared_ptr<kuiper_base::DeviceAllocator> allocator;
    if (device_type == kuiper_base::DeviceType::kDeviceCPU) {
      allocator = kuiper_base::CPUDeviceAllocatorFactory::get_instance();
    } else {
      allocator = kuiper_base::CUDADeviceAllocatorFactory::get_instance();
    }
  for (int32_t h = 0; h < head_num; ++h) {
    ...
    tensor::Tensor query_mat(kuiper_base::DataType::kDataTypeFp32, head_size, false, nullptr,
                               query_head_addr);
    ...
    for (int32_t t = 0; t <= pos; t++) {
      ...
      tensor::Tensor key_mat(kuiper_base::DataType::kDataTypeFp32, 1, head_size, false, nullptr,
                             const_cast<float*>(key_head_addr));
      
      tensor::Tensor score_mat(kuiper_base::DataType::kDataTypeFp32, 1, false, nullptr,
                               score_head_addr + t);
    ...
    tensor::Tensor score_head_tensor(kuiper_base::DataType::kDataTypeFp32, pos + 1, false, nullptr,
                                     score_head_addr);
    ...
    tensor::Tensor output_tensor(kuiper_base::DataType::kDataTypeFp32, head_size, false, nullptr,
                                 output_head_ptr);
    ...
    tensor::Tensor value_tensor(kuiper_base::DataType::kDataTypeFp32, head_size, false, nullptr,
                                value_head_addr);
