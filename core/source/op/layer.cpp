...
namespace op{
  ...
  kuiper_base::DataType BaseLayer::data_type() const { return data_type_; }

  LayerType BaseLayer::layer_type() const { return layer_type_; }
  
  kuiper_base::Status BaseLayer::set_weight(int32_t idx, const tensor::Tensor& weight) {
    return kuiper_base::error::FunctionNotImplement();
  }

kuiper_base::Status BaseLayer::set_weight(int32_t idx, const std::vector<int32_t>& dims,
                                   const void* weight_ptr, kuiper_base::DeviceType device_type) {
  return kuiper_base::error::FunctionNotImplement();
}
...
kuiper_base::DeviceType BaseLayer::device_type() const { return device_type_; }

void BaseLayer::set_device_type(kuiper_base::DeviceType device_type) { device_type_ = device_type; }



Layer::Layer(kuiper_base::DeviceType device_type, LayerType layer_type, std::string layer_name)
    : BaseLayer(device_type, layer_type, kuiper_base::DataType::kDataTypeFp32, std::move(layer_name)) {}

kuiper_base::Status Layer::init() { return kuiper_base::error::Success(); }

kuiper_base::Status Layer::forward() { return kuiper_base::error::FunctionNotImplement(""); }

kuiper_base::Status Layer::check_tensor(const tensor::Tensor& tensor, kuiper_base::DeviceType device_type,
                                 kuiper_base::DataType data_type) const {
  if (tensor.is_empty()) {
    return kuiper_base::error::InvalidArgument("The tensor parameter is empty.");
  }
  if (tensor.device_type() != device_type) {
    return kuiper_base::error::InvalidArgument("The tensor has a wrong device type.");
  }
  if (tensor.data_type() != data_type) {
    return kuiper_base::error::InvalidArgument("The tensor has a wrong data type.");
  }
  return kuiper_base::error::Success();
}

kuiper_base::Status Layer::check_tensor_with_dim(const tensor::Tensor& tensor,
                                          kuiper_base::DeviceType device_type, kuiper_base::DataType data_type,
                                          ...) const {
  std::va_list args;
  if (tensor.is_empty()) {
    return kuiper_base::error::InvalidArgument("The tensor parameter is empty.");
  ...
  for (int32_t i = 0; i < dims; ++i) {
    ...
    if (dim != tensor.get_dim(i)) {
      return kuiper_base::error::InvalidArgument("The tensor has a wrong dim in dim" + std::to_string(i));
    }
  }
  va_end(args);
  return kuiper_base::error::Success();
...
kuiper_base::Status Layer::check() const {
  return kuiper_base::error::FunctionNotImplement("The check function is not implement yet");
}
...
LayerParam::LayerParam(kuiper_base::DeviceType device_type, LayerType layer_type, bool is_quant_layer,
                       std::string layer_name)
    : Layer(device_type, layer_type, std::move(layer_name)), is_quant_layer_(is_quant_layer) {}

kuiper_base::Status LayerParam::set_weight(int32_t idx, const tensor::Tensor& weight) {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, weights_.size());
  CHECK(weight.data_type() == kuiper_base::DataType::kDataTypeFp32);
  if (!weight.is_empty()) {
...
// 传入外部权重
kuiper_base::Status LayerParam::set_weight(int32_t idx, const std::vector<int32_t>& dims,
                                    const void* weight_ptr, kuiper_base::DeviceType device_type) {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, weights_.size());
  ...
  std::shared_ptr<kuiper_base::Buffer> buffer =
      std::make_shared<kuiper_base::Buffer>(size, nullptr, const_cast<void*>(weight_ptr), true);
  if (device_type != kuiper_base::DeviceType::kDeviceUnknown) {
    buffer->set_device_type(device_type);
  }

  if (!is_quant_layer_) {
    tensor::Tensor weight(kuiper_base::DataType::kDataTypeFp32, dims);
    weight.set_device_type(device_type);
    CHECK(weight.assign(buffer));
    weights_.at(idx) = weight;
  } else {
    // is quant layer
    tensor::Tensor weight(kuiper_base::DataType::kDataTypeInt8, dims);
    weight.set_device_type(device_type);
    CHECK(weight.assign(buffer));
    weights_.at(idx) = weight;
    ...
    scales_ = tensor::Tensor{kuiper_base::DataType::kDataTypeFp32, scale_nums, false, nullptr,
...
kuiper_base::Status Layer::forward(const tensor::Tensor& input1, const tensor::Tensor& output1) {
  this->set_input(0, input1);
  this->set_output(0, output1);
  return this->forward();
}

kuiper_base::Status Layer::forward(const tensor::Tensor& input1, const tensor::Tensor& input2,
