...
static size_t data_type_size(kuiper_base::DataType data_type){
  switch(data_type){
    case kuiper_base::DataType::kDataTypeFp32: {
      return 4;
    }
    case kuiper_base::DataType::kDataTypeInt32: {
      return 4;
    }
    case kuiper_base::DataType::kDataTypeInt8: {
      return 4;
    }
    default: {
...
Tensor::Tensor(kuiper_base::DataType data_type, int32_t dim0, bool need_alloc,
                std::shared_ptr<kuiper_base::DeviceAllocator> alloc, void* ptr):data_type_(data_type){
...
Tensor::Tensor(kuiper_base::DataType data_type, int32_t dim0, int32_t dim1, bool need_alloc,
                std::shared_ptr<kuiper_base::DeviceAllocator> alloc, void* ptr):data_type_(data_type){
  dims_.push_back(dim0);
...
Tensor::Tensor(kuiper_base::DataType data_type, int32_t dim0, int32_t dim1, int32_t dim2, bool need_alloc,
                std::shared_ptr<kuiper_base::DeviceAllocator> alloc, void* ptr):data_type_(data_type){
  dims_.push_back(dim0);
  dims_.push_back(dim1);
...
Tensor::Tensor(kuiper_base::DataType data_type, int32_t dim0, int32_t dim1, int32_t dim2, int32_t dim3, bool need_alloc,
                std::shared_ptr<kuiper_base::DeviceAllocator> alloc, void* ptr):data_type_(data_type){
  dims_.push_back(dim0);
  dims_.push_back(dim1);
  dims_.push_back(dim2);
...
Tensor::Tensor(kuiper_base::DataType data_type, std::vector<int32_t> dims, bool need_alloc,
                std::shared_ptr<kuiper_base::DeviceAllocator> alloc, void* ptr)
...
void Tensor::to_cuda(cudaStream_t stream){
  ...
  if(device_type == kuiper_base::DeviceType::kDeviceUnknown){
    ...
    cu_alloc->memcpy(buffer_->ptr(),cu_buffer->ptr(),byte_size,kuiper_base::MemcpyKind::kMemcpyCPU2CUDA,stream);
...
void Tensor::to_cpu() {
  ...
  if (device_type == kuiper_base::DeviceType::kDeviceUnknown) {
    ...
    size_t byte_size = this->byte_size();
    auto cpu_alloc = kuiper_base::CPUDeviceAllocatorFactory::get_instance();
    auto cpu_buffer = std::make_shared<kuiper_base::Buffer>(byte_size, cpu_alloc);
    cpu_alloc->memcpy(buffer_->ptr(), cpu_buffer->ptr(), byte_size,
                      kuiper_base::MemcpyKind::kMemcpyCUDA2CPU);
...
bool Tensor::allocate(std::shared_ptr<kuiper_base::DeviceAllocator> allocator, bool need_realloc){
  if(!allocator){
  ...
  buffer_ = std::make_shared<kuiper_base::Buffer>(byte_size,allocator,nullptr,false);
  if (!buffer_->ptr()) {
...
bool Tensor::assign(std::shared_ptr<kuiper_base::Buffer> buffer) {
  if (!buffer) {
...
void Tensor::reshape(const std::vector<int32_t>& dims){
  ...
  if(size > size_){
    auto new_buffer = std::make_shared<kuiper_base::Buffer>(size*kuiper_base::DataTypeSize(this->data_type_),buffer_->allocator());
...
size_t Tensor::byte_size() const{
  return this->size_ * DataTypeSize(data_type_);
}
...
void Tensor::init_buffer(std::shared_ptr<kuiper_base::DeviceAllocator> alloc, kuiper_base::DataType data_type,
                         bool need_alloc, void* ptr) {
  if (!alloc && !need_alloc) {
    std::shared_ptr<kuiper_base::Buffer> buffer =
        std::make_shared<kuiper_base::Buffer>(data_type_size(data_type) * size_, nullptr, ptr, true);
    this->buffer_ = buffer;
  } else {
    allocate(alloc, true);
  }
...
kuiper_base::DeviceType Tensor::device_type() const{
  if(!buffer_){
    return kuiper_base::DeviceType::kDeviceUnknown;
  }
...
void Tensor::set_device_type(kuiper_base::DeviceType device_type) const {
  if (buffer_) {
    buffer_->set_device_type(device_type);
  }
}

void Tensor::reset(kuiper_base::DataType data_type, const std::vector<int32_t>& dims) {
...
kuiper_base::DataType Tensor::data_type() const{
